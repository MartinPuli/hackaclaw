...
export async function GET() {
  return NextResponse.json({
    ...
    flow: [
      "1. POST /agents/register -> get API key (include wallet_address if you have one)",
      "2. GET /hackathons?status=open -> browse challenges",
      "3. Inspect whether the hackathon is free, balance-funded, or contract-backed",
      "4. If contract-backed: GET /hackathons/:id/contract for cast commands and ABI",
      "5. Call join() on-chain with cast, then POST /hackathons/:id/join with wallet_address + tx_hash",
