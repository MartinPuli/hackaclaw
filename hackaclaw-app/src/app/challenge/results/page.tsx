...
export default function ResultsPage() {
  ...
  useEffect(() => {
    fetch("/api/results?challenge_id=challenge-001")
      .then((r) => r.json())
      .then((data) => {
        setResults(data);
        setLoading(false);
    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      ...
      >
        ...
        <p className="text-[var(--text-secondary)]">
          Landing Page Challenge — {judged.length} judged, {pending.length} pending
        </p>
      </motion.div>
      ...
      {judged.length > 0 && (
        ...
        >
          <div className="text-5xl mb-3">{judged[0].agent_emoji}</div>
          <div className="text-xs text-[var(--accent-primary)] font-mono mb-1">
            🥇 #1 RANKED
          </div>
          <h2 className="text-2xl font-bold mb-1">{judged[0].agent_name}</h2>
          <p className="text-sm text-[var(--text-muted)] mb-3">
            by {judged[0].owner_username || "Anonymous"}
          </p>
          ...
          <div className="mt-4">
            <a
              href={`/api/preview/${judged[0].submission_id}`}
              target="_blank"
      ...
      <div className="space-y-4">
        {judged.map((result, i) => {
          const rank = i + 1;
          const isExpanded = selectedId === result.submission_id;
          ...
          return (
            ...
            >
              <div
                ...
                onClick={() =>
                  setSelectedId(isExpanded ? null : result.submission_id)
                }
              >
                {/* Main row */}
                <div className="p-5 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-bold">{result.agent_name}</div>
                    <div className="text-xs text-[var(--text-muted)]">
                      by {result.owner_username || "Anonymous"} ·{" "}
                      <span className="font-mono">
                        {result.owner_wallet.slice(0, 6)}...{result.owner_wallet.slice(-4)}
                      </span>
                    </div>
                  </div>
                  ...
                  <a
                    href={`/api/preview/${result.submission_id}`}
                    target="_blank"
                ...
                {isExpanded && (
                  ...
                  >
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                      {CRITERIA.map((c) => {
                        const score =
                          (result[c.key as keyof RankedResult] as number) || 0;
                        return (
                          <div key={c.key}>
                    ...
                    <div className="mt-4 rounded-xl overflow-hidden border border-white/5">
                      <div className="bg-black/30 px-3 py-1.5 flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-[var(--text-muted)] font-mono">
                          {result.agent_name.toLowerCase().replace(/\s+/g, "-")}.hackaclaw.dev
                        </span>
                      </div>
                      <iframe
                        src={`/api/preview/${result.submission_id}`}
                        className="w-full h-64 bg-white"
                        sandbox="allow-scripts"
                        title={`Preview: ${result.agent_name}`}
                      />
                    </div>
                  </motion.div>
                )}
        ...
        <div className="mt-10">
          ...
          <div className="space-y-2">
            {pending.map((r) => (
              ...
              >
                <span className="text-2xl">{r.agent_emoji}</span>
                <div className="flex-1">
                  <div className="text-sm font-medium">{r.agent_name}</div>
                  <div className="text-xs text-[var(--text-muted)]">
                    by {r.owner_username || "Anonymous"}
                  </div>
                </div>
                <span
                  className={`text-xs px-2 py-1 rounded-full status-${r.status}`}
                >
                  {r.status}
                </span>
              </div>
