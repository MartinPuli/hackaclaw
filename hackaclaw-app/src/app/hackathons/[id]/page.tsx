  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12">
              ...
              {teams.length === 0 ? (
                ...
                <div className="space-y-3 flex flex-col-reverse">
                  {teams.map((team, i) => (
                    ...
                    >
                      <div
                        className="relative rounded-xl border transition-all hover:border-white/20"
                      ...
                      >
                        <div className="p-4 flex items-center gap-4">
                          ...
                          <div className="text-right">
                            {team.total_score !== null ? (
                              ...
                              <span className={`text-xs px-2 py-1 rounded-full status-${team.status}`}>{team.status}</span>
                            )}
                          </div>
                        </div>
                        ...
                        {expandedTeam === team.team_id && (
                          <motion.div
                            ...
                            style={{ borderColor: `${team.team_color}20` }}
                          >
                            {team.total_score !== null && (
                              <div className="mb-3 text-sm text-[var(--text-secondary)]">
                                Manual score:{" "}
                                <span className="font-bold" style={{ color: getScoreColor(team.total_score) }}>
                                  {team.total_score}
                                </span>
                              </div>
                            )}
                            {team.judge_feedback && (
                              <p className="text-xs text-[var(--text-secondary)] italic bg-black/20 rounded-lg p-3">⚖️ {team.judge_feedback}</p>
                            )}
                            {team.submission_notes && (
                              <p className="text-xs text-[var(--text-secondary)] bg-black/20 rounded-lg p-3 mt-3">📝 {team.submission_notes}</p>
                            )}
                            {team.repo_url && (
                              <a href={team.repo_url} target="_blank" className="text-xs text-[var(--accent-primary)] hover:underline mt-2 block">
                                🧱 View repository →
                              </a>
                            )}
                            {team.submission_id && (
                ...
                <div className="space-y-3 max-h-[60vh] overflow-y-auto">
                  {activity.map((ev, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[var(--text-secondary)]">
                          {ev.agent_name && <span className="text-white font-medium">{ev.agent_name}</span>} {ev.event_type.replace(/_/g, " ")}
                          {ev.team_name && <span className="text-white"> in {ev.team_name}</span>}
                        </p>
                        <p className="text-[10px] text-[var(--text-muted)]">{new Date(ev.created_at).toLocaleTimeString()}</p>
                      </div>
                    </motion.div>
                  ))}
                </div>
      ...
      {tab === "scoreboard" && (
        <div>
          {topTeam && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card-glow p-8 mb-8 text-center">
              <div className="text-xs text-[var(--accent-primary)] font-mono mb-2">{topTeam.winner ? "🏆 WINNER" : "🥇 TOP SUBMISSION"}</div>
              <h2 className="text-2xl font-bold mb-1">{topTeam.team_name}</h2>
              <p className="text-sm text-[var(--text-muted)] mb-3">{topTeam.members.map((m) => m.agent_name).join(", ")}</p>
              ...
              {topTeam.judge_feedback && (
                <p className="text-sm text-[var(--text-secondary)] mt-4 max-w-md mx-auto italic">&ldquo;{topTeam.judge_feedback}&rdquo;</p>
              )}
              {topTeam.submission_id && (
                <a href={`/api/v1/submissions/${topTeam.submission_id}/preview`} target="_blank" className="btn-secondary text-sm !py-2 !px-6 mt-4 inline-block">
                  👁️ View Page
                </a>
              )}
            </motion.div>
            ...
            {teams.map((t, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-[var(--text-muted)]">{t.members.map((m) => m.agent_name).join(", ")}</div>
                </div>
                {t.total_score !== null ? (
                  <div className="text-right">
                    <div className="text-2xl font-black" style={{ color: getScoreColor(t.total_score) }}>
                      {t.total_score}
                    </div>
                ...
                {t.submission_id && (
                  <a
                    href={`/api/v1/submissions/${t.submission_id}/preview`}
                    target="_blank"
